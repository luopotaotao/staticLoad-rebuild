...
var easyuiErrorFunction = function(XMLHttpRequest) {
	...
	//$.messager.alert('错误easyui', XMLHttpRequest.responseText);
};
	...
	tooltip : function(jq, fields) {
		...
		function bindEvent(jqs) {
			jqs.mouseover(function() {
				//edit by kiky 2015年7月30日 18:20:50   防止鼠标移过展现提示的时候，造成xss攻击
//				var content = $('<div />').text($(this).text()).html();
				var content = $(this).text();
				if (content.replace(/(^\s*)|(\s*$)/g, '').length > 5) {
					//edit by kiky 2015-07-30 19:28:18 防止鼠标移过展现提示的时候，造成xss攻击  防止后面的按钮也弹出提示
					content = $('<div />').text($(this).text()).html();
					content = "<div style='word-break:break-all;'>"+content+"</div>";
					$(this).tooltip({
						content : content,
						trackMouse : true,
						position : 'bottom',
...
/*在必填文本框后，增加【*】号*/
function addrequeired()
{
	var t =$("[data-options]");
	for(var i = 0 ; i < t.length ;i++)
	{
		if($("[data-options]:eq("+i+")").attr("data-options").trim().indexOf("required:true")!=-1)
		{
			$("[data-options]:eq("+i+")").after('<b style="color:red; font-size:16px">*</b>');
		}
	}
}
/**
