...
$.cookie = function(key, value, options) {
	...
	if (arguments.length > 1 && (value === null || typeof value !== "object")) {
		...
		//add by zhangxh 2015-7-28 15:05:12
		options.secure = true;
		if (value === null) {
			options.expires = -1;
		}
...
$.ajaxSetup({
	...
	error : function(XMLHttpRequest, textStatus, errorThrown) {
		try {
			...
			{parent.$.messager.alert('提示', "服务器无响应，请联系管理员");}
			else
			{
				if(XMLHttpRequest.responseText.indexOf("您还没有登录或登录已超时")!=-1)
				{
					parent.$.messager.alert('提示','您还没有登录或登录已超时，请重新登录！','info',function(){
						parent.location.href='./';
					});
				}
				else
				{
					parent.$.messager.alert('提示', XMLHttpRequest.responseText);
				}
			}
