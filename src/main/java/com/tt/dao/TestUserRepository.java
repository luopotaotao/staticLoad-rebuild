...
////        Inspector user = (Inspector) getSession().createCriteria(Inspector.class).add(Restrictions.eq("username",username)).uniqueResult();
...
//        return user;//(Inspector)getSession().createCriteria(Inspector.class).add(Restrictions.eq("id", Property.forName("id").max())).uniqueResult();
//    }
//}
